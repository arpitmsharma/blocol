...
package io.g33k.blocol.clustering.hac.visualization;
...
import io.g33k.blocol.clustering.hac.DefaultClusteringAlgorithm;
...
public class DendrogramPanel extends JPanel {
	...
	private ClusterComponent createComponent(Cluster cluster, VCoord initCoord,
		...
		if (cluster != null) {
			...
			double distance = cluster.getDistanceValue() == null ? 0 : cluster
					.getDistanceValue();
			for (Cluster child : cluster.getChildren()) {
				int childLeafCount = child.countLeafs();
				double childHeight = childLeafCount * leafHeight;
				double childDistance = child.getDistanceValue() == null ? 0
						: child.getDistanceValue();
				VCoord childInitCoord = new VCoord(initCoord.getX()
						+ (distance - childDistance), yChild + childHeight
						/ 2.0);
				yChild += childHeight;

				/* Traverse cluster node tree */
				ClusterComponent childComp = createComponent(child,
						childInitCoord, childHeight);
	...
	private ClusterComponent createComponent(Cluster model) {
		...
		ClusterComponent comp = createComponent(model, initCoord,
				virtualModelHeight);
	...
	public void paint(Graphics g) {
		...
		if (component != null) {
			...
			int nameGutterWidth = component.getMaxNameWidth(g2, false)
					+ component.getNamePadding();
			wDisplay -= nameGutterWidth;

			if (showScale) {
				Rectangle2D rect = g2.getFontMetrics().getStringBounds("0", g2);
				int scaleHeight = (int) rect.getHeight() + scalePadding
						+ scaleTickLength + scaleTickLabelPadding;
			...
			if (showScale) {
				...
				double totalDistance = component.getCluster()
						.getTotalDistance();
				...
				while (xTick >= xDisplayOrigin) {
					...
					String distanceValueStr = String.format("%."
							+ scaleValueDecimals + "f", distanceValue);
					Rectangle2D rect = g2.getFontMetrics().getStringBounds(
							distanceValueStr, g2);
					g2.drawString(distanceValueStr,
							(int) (xTick - (rect.getWidth() / 2)), y2
									- scaleTickLabelPadding);
